from utils.evaluation import eval_game, eval_relative, eval_game_T
...
from utils.functions import MSECP
...
def train_collate(batch):
    ...
    if type(transposed_batch[0][0]) == list:
        ...
        # ostu_list=[item[2] for item in transposed_batch[0]]
        rgb = torch.stack(rgb_list, 0)
        t = torch.stack(t_list, 0)
        # ostu = torch.stack(ostu_list, 0)
        images = [rgb,t]
    else:
        images = torch.stack(transposed_batch[0], 0)
    points = transposed_batch[1]  # the number of points is not fixed, keep it as a list of tensor
    st_sizes = torch.FloatTensor(transposed_batch[2])
    # target= torch.FloatTensor(transposed_batch[3])
...
class RegTrainer(Trainer):
    def setup(self):
        ...
        self.num_steps_per_epoch = len(self.dataloaders)
        ...
        #                                T_max=600# * self.num_steps_per_epoch,
        #                         )
    ...
    def train_eopch(self):
        ...
        logging.info('Epoch {} Train, Loss: {:.4f}, GAME0: {:.2f} MSE: {:.2f}, Cost {:.1f} sec'
